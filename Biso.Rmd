...
age <-  mean(mydata$age, na.rm = T)
sex <- c("male", "female") 
family.income <- mean(mydata$family.income, na.rm = T)
weapon.carry <- mean(mydata$weapon.carry, na.rm = T)
...
predictions <- as.tibble(expand.grid(age, sex, family.income, weapon.carry, network.delinquency, popularity, centrality, density))   
colnames(predictions) <- c("age", "sex", "family.income",  "weapon.carry", "network.delinquency", "popularity", "centrality", "density")      
#predictions$joinby <- seq(1:nrow(predictions))                       

#Predicted Likelihood of Victimization added to the newdata 
predictions$victim <- predict(imodel2, newdata = predictions, type = "response")

predictions <- predictions %>%
  select(network.delinquency, victim, centrality)

#Visualize
  ggplot(data = predictions, aes(x = predictions$network.delinquency, y = predictions$victim, group = predictions$centrality)) +
  geom_smooth(aes(color = predictions$centrality), method = "lm" )

ggplot(predictions, aes(x = network.delinquency, y = victim)) + 
  geom_line(aes(colour = centrality))
summary(imodel2)
```
newdata <- mydata.c %>%
  select(age, sex, family.income, weapon.carry, network.delinquency, popularity, centrality, density) %>%
  mutate(age = mean(age),
         sex = factor(sex),
         ...
         centrality = quantile(centrality, c(.1, .5, .9)),
         density = mean(denisty))
