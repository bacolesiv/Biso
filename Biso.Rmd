...
#Mac
load("~/desktop/Biso/Biso/data/wave1.rda")
load("~/desktop/Biso/Biso/data/network.wave1.rda")
load("~/desktop/Biso/Biso/data/weights.rda")
...
```{r}
#Constructing Race Dummy Variable -> you can use the recode function to condense and maybe improve this code? 
#recode is good for calapsing a single variable into smaller categories
mydata$race <- NA 
for(i in 1:6504) {
...
mydata$weapon.carry.na <- as.numeric(mydata$weapon.carry.na)
```

###Trying out a Map Model  
```{r}
victim.na <- str_remove(mydata$victim, " NA$")
weapon.carry.na <- str_remove(mydata$weapon.carry, " NA$")

model <- map(
  alist(
    victim.na ~ dbinom(1,p),
    logit(p) <- a + bp*weapon.carry,
    a ~ dnorm(0, 10),
    bp ~ dnorm(0, 10)),
  data = mydata)
...
```{r}
hues <- c("red", "blue", "green", "yellow")

my_data <- tibble(lets = sample(letters, 1000, replace = TRUE),
               nums = sample(1:5, 1000, replace = TRUE),
               color = sample(hues, 1000, replace = TRUE))

my_data

my_data  <-  my_data %>%
    mutate(new_var = case_when(
               lets %in% letters[1:13] &
               nums > 2 & color %in% c("red", "yellow") ~ "Yes",
               TRUE ~ "No")
           )
