...
```{r}
racial.heterogeneity <- quantile(mydata.c$racial.heterogeneity, c(.1, .5, .9))
  
mydata.c <- mydata.c %>%
  mutate(rh.cat = case_when(
    racial.heterogeneity <= .2 ~ "0:10th Percentile",
    (racial.heterogeneity > .2 & racial.heterogeneity <= .4) ~ "10th:50th Percentile",
    (racial.heterogeneity > .4 & racial.heterogeneity <= .75) ~ "50th:90th Percentile",
    racial.heterogeneity > .75 ~ ">90th Percentile",
    T ~ NA_character_))

racial.heterogeneity.levels <- c("0:10th Percentile", "10th:50th Percentile", "50th:90th Percentile", ">90th Percentile")
...
rh.victim <- mydata.c %>%
  ...
  geom_hline(yintercept= mean(mydata.c$weapon.carry), linetype = "dashed", color = "darkred") +
  ylim(0, .1) + 
  annotate("text", x = 3.2, y = 0.063, label = "Mean", color = "darkred") +
  coord_flip() +
  labs(title = "Racial Heterogeneity", y = "Proportion of Victims", x = " ") +
  theme_igray() +
  theme(text = element_text(family = "Times"), plot.title = element_text(hjust = 0.5), legend.position = "none") 

ggsave("rh_plot.pdf", rh.victim, width = 25, height = 15, units = "cm")
```
