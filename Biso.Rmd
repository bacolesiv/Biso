...
samples <- full_join(sample, predictions, by = "joinby") %>%             
  mutate(victim = exp(a + b1*weapon.carry + b2*network.delinquency + b3*popularity + b4*centrality + b5*density + b6*network.delinquency*centrality)) %>%         
  group_by(network.delinquency, centrality) %>%                              
  mutate(p = mean(victim),                                  
        lb.p = HPDI(victim, prob = .89)[1],                 
        ub.p = HPDI(victim, prob = .89)[2]) %>%             
  slice(1)
...
ggplot(samples, aes(x = network.delinquency , group = Centrality)) +
  geom_line(aes(y = p, color = Centrality), size = 1.5) +
  geom_ribbon(aes(ymin = lb.p, ymax = ub.p) , alpha = .2)  +
  labs(title = "Adolescent Victimization", x = "Average Delinquency in Friendship Network", y = "Percent Increase in Likelihood of Victimization")
```
...
samples <- full_join(sample, predictions, by = "joinby") %>%             
  mutate(victim = exp(a + b1*weapon.carry + b2*network.delinquency + b3*popularity + b4*centrality + b5*density + b6*network.delinquency*popularity)) %>%         
...
ggplot(samples, aes(x = network.delinquency , group = Popularity)) +
  geom_line(aes(y = p, color = Popularity) , size = 1.5) +
  geom_ribbon(aes(ymin = lb.p, ymax = ub.p) , alpha = .2)  +
  labs(title = "Adolescent Victimization", x = "Average Delinquency in Friendship Network", y = "Percent Increase in Likelihood of Victimization")
```
...
samples <- full_join(sample, predictions, by = "joinby") %>%             
  mutate(victim = exp(a + b1*weapon.carry + b2*network.delinquency + b3*popularity + b4*centrality + b5*density + b6*network.delinquency*density)) %>%         
...
ggplot(samples, aes(x = network.delinquency , group = Density)) +
  geom_line(aes(y = p, color = Density) , size = 1.5) +
  geom_ribbon(aes(ymin = lb.p , ymax = ub.p) , alpha = .2)  +
  labs(title = "Adolescent Victimization", x = "Average Delinquency in Friendship Network", y = "Percent Increase in Likelihood of Victimization")
```
