...
summary1 <- list(
       "Victim" =
       list("Min" = ~ min(mydata.c$victim),
            "Max" = ~ max(mydata.c$victim),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$victim)),
       "Age" =
       list("Min" = ~ min(mydata.c$age),
            "Max" = ~ max(mydata.c$age),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$age)),
       "Sex" =
       list("Min" = ~ min(mydata.c$female),
            "Max" = ~ max(mydata.c$female),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$female)),
       "Family Income" =
       list("Min" = ~ min(mydata.c$family.income),
            "Max" = ~ max(mydata.c$family.income),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$family.income)),
       "Weapon Carrying" =
       list("Min" = ~ min(mydata.c$weapon.carry),
            "Max" = ~ max(mydata.c$weapon.carry),
            "Mean (Stdvd)" = ~ qwraps2::mean_sd(mydata.c$weapon.carry)),
       "Network Delinquency" =
       list("Min" = ~ min(mydata.c$network.delinquency),
            "Max" = ~ max(mydata.c$network.delinquency),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$network.delinquency),
       "Centrality" =
       list("Min" = ~ min(mydata.c$centrality),
            "Max" = ~ max(mydata.c$centrality),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$centrality)),
       "Popularity" =
       list("Min" = ~ min(mydata.c$popularity),
            "Max" = ~ max(mydata.c$popularity),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$popularity)),
       "Density" =
       list("Min" = ~ min(mydata.c$density),
            "Max" = ~ max(mydata.c$density),
            "Mean (Stdv)" = ~ qwraps2::mean_sd(mydata.c$density))))

#Making the Table object
options(qwraps2_markup = "markdown")

mydata2 <- as.data.frame(mydata.c)

table <- qwraps2::summary_table(mydata2, summary1)
args(summary_table)
